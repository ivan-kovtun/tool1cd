...
RUN unzip /root/linux.zip -d /root/tool-1cd \
    && unzip /root/template.zip -d /root/template \
...
RUN apt update -y
RUN apt install libboost-all-dev libboost-filesystem-dev libboost-system-dev libboost-regex-dev zlib1g-dev libqt6core5compat6-dev -y
